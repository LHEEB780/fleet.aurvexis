...
/**
 * Pre-loads and converts external avatar image into safe, local DataURL
 * to avoid canvas tainting (CORS SecurityError) during html2canvas rendering.
 */
async function getSafeAvatarDataUrl(url: string | undefined): Promise<string | null> {
...
export interface DriverPdfExportOptions {
  safeMode?: boolean;
}
...
export async function exportDriverProfilePDF(
  ...
  language: 'ar' | 'en' = 'ar',
  options: DriverPdfExportOptions = {}
): Promise<void> {
  ...
  try {
    ...
    const qrString = `AURVEXIS-DRIVER-PROFILE:${driver.id}:${driver.identityNumber || 'UNKNOWN'}:${new Date().toISOString()}`;
    let qrDataUrl = '';
    try {
    ...
    const getDepartmentLabel = (dept?: string) => {
      if (!dept) return isAr ? 'إدارة الحركة والنقل' : 'Fleet Operations';
      if (!isAr) {
        switch (dept) {
    ...
    // Safely resolve avatar image to prevent canvas tainting
    currentPhase = 'AVATAR_PROCESSING';
    let safeAvatarUrl: string | null = null;
    if (!options.safeMode && driver.avatar) {
      try {
        safeAvatarUrl = await getSafeAvatarDataUrl(driver.avatar);
      } catch (e) {
        console.warn('Avatar safe conversion failed, fallback to initials:', e);
      }
    }
    ...
    // Avatar visual representation (pure CSS/DataURL to prevent CORS taint)
    const avatarHtml = safeAvatarUrl
      ? `<img src="${safeAvatarUrl}" alt="Avatar" crossOrigin="anonymous" style="width: 100%; height: 100%; object-fit: cover;" />`
      : `<div style="font-size: 26px; font-weight: 900; color: #4338ca; text-align: center; line-height: 84px; background: #e0e7ff; width: 100%; height: 100%;">
          ${(driver.name || 'D').slice(0, 2).toUpperCase()}
        </div>`;

    currentPhase = 'DOM_TEMPLATE_CREATION';
    const htmlContent = `
      <div style="
        width: 794px;
        min-height: 1123px;
        ...
        ">
          ...
          <div style="flex: 1;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
              <div>
                <div style="font-size: 17px; font-weight: 900; color: #0f172a;">${driver.name || (isAr ? 'سائق غير مسمى' : 'Unnamed Driver')}</div>
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px 14px; margin-top: 10px; font-size: 10px;">
              ...
              <div>
                ...
                <div style="font-weight: 800; color: #1e293b; margin-top: 2px;">${getLicenseTypeLabel(driver.licenseType)}</div>
              </div>
              <div>
    ...
    try {
      ...
      // allowTaint MUST be false to prevent SecurityError when calling toDataURL
      ...
      try {
        ...
        console.warn('Canvas export failed due to tainting, retrying safe export:', taintErr);
        throw new Error(`[CanvasExportError] Canvas tainted or restricted: ${taintErr?.message || taintErr}`);
      }
      ...
      // Generate safe file name ensuring ASCII safety in English mode
      const safeEnDriverName = cleanDriverName.replace(/[^\x00-\x7F]/g, '').trim() || `Driver_${driver.id}`;
      const fileName = isAr
        ? `تقرير_الأداء_الميداني_للسائق_${cleanDriverName}.pdf`
        : `Driver_Field_Profile_${safeEnDriverName}.pdf`;
    ...
    const errorDetails = new Error(
      `[DriverReportPDF:${currentPhase}] ${rawError?.name || 'Error'}: ${rawError?.message || String(rawError)}`
    );
    (errorDetails as any).phase = currentPhase;
    (errorDetails as any).originalError = rawError;
    (errorDetails as any).language = language;
    (errorDetails as any).driverId = driver.id;
    throw errorDetails;
  }
}
